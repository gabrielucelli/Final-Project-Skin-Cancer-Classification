...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('w1'):
      ...
      layer_w1 = tf.Variable(initial_value1, name='w1')
      variable_summaries(layer_w1)

    with tf.name_scope('b1'):

      layer_b1 = tf.Variable(tf.zeros([bottleneck_tensor_size]), name='b1')
      variable_summaries(layer_b1)

    with tf.name_scope('W1_plus_b1'):

      w1b1 = tf.matmul(bottleneck_input, layer_w1) + layer_b1
      tf.summary.histogram('w1b1', w1b1)
    ...
    with tf.name_scope('w2'):
      ...
      layer_w2 = tf.Variable(initial_value2, name='w2')
      variable_summaries(layer_w2)

    with tf.name_scope('b2'):

      layer_b2 = tf.Variable(tf.zeros([class_count]), name='b2')
      variable_summaries(layer_b2)

    with tf.name_scope('W2_plus_b2'):

      logits = tf.matmul(w1b1, layer_w2) + layer_b2
      tf.summary.histogram('pre_activations', logits)

  ## (positive image) 3315 / (total images) 4386 = 0,755813953
  ## 0,244186047
  ...
  with tf.name_scope('cross_entropy'):
    ...
    classes_weights = tf.constant([0.755813953])

    cross_entropy = tf.nn.weighted_cross_entropy_with_logits(
        logits=logits, targets=ground_truth_input, pos_weight=classes_weights)
